import { writeRadioCell, getIndicesFromRadioName } from './createScoringTable';
...
describe('create scoring table', () => {
  describe('radio buttons', () => {
    const getRadioButtonsChecked = (html) => {
      ...
      return Array.prototype.reduce.call(inputElements, (acc, el) => {
        if(el.value === 'Y') { acc['yesRadioChecked'] = el.hasAttribute('checked'); }
        if(el.value === 'N') { acc['noRadioChecked'] = el.hasAttribute('checked'); }
        return acc;
      },{});
    }
    ...
    test('returns radio button with yes checked when score greater than zero', () => {
      ...
      const { yesRadioChecked, noRadioChecked } = getRadioButtonsChecked(radiosHtml);
    ...
    test('parses id into question and team indices', () => {
      ...
      const { qIndex, teamIndex } = getIndicesFromRadioName(`team${teamIndexExpect}_Q${qIndexExpect}`);
